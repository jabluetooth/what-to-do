...
- **Interaction:**
  ...
  4. "Randomize again" replaces the card with a new random idea; this can repeat indefinitely with no cap in v1.
...
- **Abuse prevention:** Prompts and generated ideas pass through basic content moderation before reaching the LLM pipeline (block clearly malicious asks — phishing kits, credential harvesters, spam/scraper tooling). PRD and stack-recommendation calls are rate-limited per guest session, not only preview generation.
- **Cost control:** Preview environments must have hard timeouts and idle shutdown (see 6.5). Guest sessions and free-tier signed-in users are subject to a fixed cap on pipeline generations per session/day — a firm requirement rather than optional, since guest access has no sign-up gate to fall back on. Exact cap numbers are TBD pending the pricing model (see 10).
...
| Pricing model | Not yet defined; affects whether guest sandbox and free signed-in tier need hard usage caps at launch. |
