 ...
 * Plays only when `play` changes to a new (non-zero) value — a fresh idea landing bumps a
 * counter in the parent, this never auto-loops or animates on mount. Real text is never
 * generated by this component, only scrambled cosmetically in place.
 */
function TextScramble({ text, play, className }: { text: string; play: number; className?: string }) {
  const [display, setDisplay] = useState(text);
  const frameRef = useRef<number | null>(null);
  const lastPlayRef = useRef(play);

  useEffect(() => {
    // Only animate when `play` actually changed since the last run — a `text` change alone
    // (e.g. resetting to a placeholder with no fresh idea) just swaps the display instantly
    // instead of leaving it stuck on stale scrambled characters.
