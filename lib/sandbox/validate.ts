...
/**
 * FastAPI counterpart to validateBoilerplate. Deliberately does NOT pip install or run the
 * generated app: this is a Node.js serverless function (Vercel), and there's no guarantee a
 * Python interpreter — let alone one with network access to pip-install a real dependency set —
 * is even present in that runtime, unlike Node/pnpm which the function's own runtime already
 * needs to exist. Checks only that the LLM's Python is syntactically valid (`py_compile`, stdlib
 * only, no dependencies needed) and degrades gracefully — `unvalidated: true`, not a failure —
 * when no Python interpreter is found at all, rather than blocking every FastAPI generation on a
 * platform capability this validator can't assume. This is a materially weaker guarantee than
 * the Next.js path's real install+build (see that template's README, which says so explicitly).
 */
